...
var defaultRequest = fosite.Request{
	...
	Client:            &models.Client{OutfacingID: "foobar"},
...
func mockRequestForeignKey(t *testing.T, id string, x InternalRegistry, createClient bool) {
	cl := &models.Client{OutfacingID: "foobar"}
...
func testHelperRevokeRefreshToken(x InternalRegistry) func(t *testing.T) {
	return func(t *testing.T) {
		...
		err = m.CreateRefreshTokenSession(ctx, "1111", &fosite.Request{ID: reqIdOne, Client: &models.Client{OutfacingID: "foobar"}, RequestedAt: time.Now().UTC().Round(time.Second), Session: &Session{}})
		require.NoError(t, err)

		err = m.CreateRefreshTokenSession(ctx, "1122", &fosite.Request{ID: reqIdTwo, Client: &models.Client{OutfacingID: "foobar"}, RequestedAt: time.Now().UTC().Round(time.Second), Session: &Session{}})
		require.NoError(t, err)
...
func testHelperNilAccessToken(x InternalRegistry) func(t *testing.T) {
	return func(t *testing.T) {
		m := x.OAuth2Storage()
		c := &models.Client{OutfacingID: "nil-request-client-id-123"}
